...
js/
  ...
  views/
    ...
    meetView.js           # встречи (режимы день/неделя/месяц/список)
...
- Встречи показываются в виде минимальной карточки (время + тема), а полные детали отображаются в popover из `meetView`.
- Верхняя зона встреч (панель управления + строка дней) обёрнута в sticky-контейнер, чтобы оставаться видимой при прокрутке.
- Для форматирования диапазонов дат/времени используются функции из `utils/dateTime.js`, без локальных дубликатов.
...
- Все Pyrus ID (формы/поля) должны находиться строго в `config.json`.
- Запрещено использовать хардкод-числа ID в коде (Services/Views). Доступ — только через typed getters в `config.js`.

## Lifecycle cleanup

- Любой контроллер, подписывающийся на глобальные события (`document`/`window`), обязан иметь метод `destroy()`.
- `destroy()` должен вызываться при unmount view или смене режима отображения во избежание утечек памяти и дублирования слушателей.

## Cookie safety

- В cookies храним только компактный профиль пользователя (ID, имя, роли).
- **Запрещено** сохранять в cookies base64-аватары или длинные URL. Аватары хранятся только в in-memory кеше или `RequestCache`.

## KP Module

- **Интеграция**: Все данные (справочники, сделки, сохранение) конфигурируются строго через `config.json`.
- **HTML генерация**: Публичный HTML должен быть самодостаточным.
- **Восстановление**: Модель данных для редактирования (JSON) встраивается в HTML внутри тега `<script id="kp-data-b64" type="application/json">` в формате Base64.
- **Изоляция**: Публичный файл не должен зависеть от внешних CSS/JS (кроме минимальных inline стилей).
