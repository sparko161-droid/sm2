...
В `config.json` можно задать `routeAccess`:

- пустой массив `[]` — доступно всем
- непустой массив — доступ только указанным ролям/ID

Если маршрут недоступен, вкладка скрывается в шапке, а при ручном вводе hash показывается оверлей с сообщением о запрете и кнопкой возврата на `#work`.

## Переходы по умолчанию

- При открытии базовой ссылки без hash приложение перенаправляет на `#login`, если нет активной сессии.
- Если сессия есть, базовая ссылка ведёт на `#work`.
- Переход на `#login` с активной сессией редиректит на `#work`.
